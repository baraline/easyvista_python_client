...
@respx.mock
def test_list_actions_sends_the_configured_row_cap(config):
    ...
    with EasyvistaClient(config) as client:
        ...
        client.list_actions("I240101_0001")
...
@respx.mock
def test_update_action_sends_a_put_to_the_top_level_path(config):
    ...
    with EasyvistaClient(config) as client:
        action = client.update_action(57483, ActionUpdate(description="edited"))
...
@respx.mock
def test_delete_document_sends_a_delete_to_the_nested_path(config):
    ...
    with EasyvistaClient(config) as client:
        result = client.delete_document("I240101_0001", "12345_abcdef")
    assert route.call_count == 1
...
@respx.mock
def test_get_department_comment_honours_a_memo_field_override(config):
    ...
    with EasyvistaClient(config) as client:
        client.get_department_comment(60, memo_field="comment_service")
...
@respx.mock
def test_stream_document_chunks_reassemble_to_the_download(config):
    ...
    chunks = []
    with EasyvistaClient(config) as client:
        for chunk in client.stream_document(doc, chunk_size=512):
            chunks.append(chunk)
...
@respx.mock
def test_stream_document_closes_the_inner_stream_when_stopped_early(config):
    ...
    with EasyvistaClient(config) as client:
        ...
        assert chunks.__next__() == b"01234567"
...
@respx.mock
def test_stream_document_accepts_a_relative_path_like_download_document(config):
    ...
    with EasyvistaClient(config) as client:
        streamed = [chunk for chunk in client.stream_document(path)]
        downloaded = client.download_document(path)
    assert b"".join(streamed) == downloaded == b"bytes"


@respx.mock
def test_stream_document_and_download_document_agree_on_a_403(config):
    ...
    with EasyvistaClient(config) as client:
        with pytest.raises(EasyvistaError) as streamed:
            [chunk for chunk in client.stream_document(doc)]
...
def test_stream_document_refuses_a_foreign_download_url(config):
...
def test_stream_document_needs_a_download_url(config):
...
@respx.mock
def test_ticket_statistics_aggregates_across_pages(config):
...
@respx.mock
def test_ticket_statistics_passes_languages_through_to_the_aggregator(config):
    ...
    with EasyvistaClient(config) as client:
        default = client.ticket_statistics(dimensions=["STATUS"])
        french = client.ticket_statistics(
...
@respx.mock
def test_find_departments_auto_tries_code_before_id(config):
...
@respx.mock
def test_find_departments_auto_falls_back_to_id_for_an_all_digit_name(config):
...
@respx.mock
def test_find_departments_by_pins_a_single_column(config):
    ...
    with EasyvistaClient(config) as client:
        found = client.find_departments("60", by="DEPARTMENT_ID")
    assert [d.department_id for d in found] == [60]
    assert route.call_count == 1
...
@respx.mock
def test_get_ticket_context_still_raises_on_a_404_from_a_list_call(config):
...
@respx.mock
def test_get_ticket_context_resolves_the_two_default_memos(config):
    respx.get(f"{ROOT}/requests/I1").mock(
        return_value=httpx.Response(200, json={"RFC_NUMBER": "I1"})
    )
    respx.get(f"{ROOT}/requests/I1/description").mock(
    ...
    with EasyvistaClient(config) as client:
        ctx = client.get_ticket_context("I1", resolve_action_bodies=False)
...
@respx.mock
def test_get_ticket_context_honours_custom_memo_fields(config):
...
@respx.mock
def test_get_department_context_honours_memo_fields(config):
...
@respx.mock
def test_get_department_context_projects_and_sorts_recent_tickets(config):
    ...
    with EasyvistaClient(config) as client:
        client.get_department_context(60, resolve_manager=False)
    sweeps = [
...
@respx.mock
def test_get_department_context_forwards_a_caller_projection_and_sort(config):
...
@respx.mock
def test_get_department_context_caps_the_statistics_sample(config):
...
@respx.mock
def test_get_department_context_records_degraded_branches(config, status):
    ...
    with EasyvistaClient(config) as client:
        ctx = client.get_department_context(60, resolve_manager=False)
    for branch in (
        "employees",
        "recent_tickets",
        "assets",
...
@respx.mock
def test_send_reaches_an_unwrapped_route_and_returns_raw_json(config):
    ...
    with EasyvistaClient(config) as client:
        assert client.send("GET", "status") == {"records": [{"STATUS_ID": 8}]}
    assert route.call_count == 1


@respx.mock
def test_send_upper_cases_the_method_and_strips_a_leading_slash(config):
...
@respx.mock
def test_send_shares_the_error_mapping_and_retry_policy(config):
...
def test_send_puts_a_bare_list_body_on_the_wire(config):
    ...
    with EasyvistaClient(config) as client:
        client.send("POST", "groups", json=[{"a": 1}])
    assert json.loads(route.calls.last.request.content) == [{"a": 1}]


@respx.mock
def test_send_never_reaches_a_foreign_host(config):
    ...
    with EasyvistaClient(config) as client:
        with pytest.raises(EasyvistaError):
            client.send("GET", "https://attacker.test/steal")
...
@respx.mock
def test_search_tickets_sends_params_alongside_the_builders_own(config):
    route = respx.get(f"{ROOT}/requests").mock(
        return_value=httpx.Response(200, json={"records": []})
    )
    with EasyvistaClient(config) as client:
        client.search_tickets(params={"formatDate": "iso"})
...
@respx.mock
def test_a_caller_param_cannot_replace_the_builders_own(config):
    ...
    with EasyvistaClient(config) as client:
        client.search_tickets(max_rows=10, params={"max_rows": 5})
    assert route.calls.last.request.url.params["max_rows"] == "10"


@respx.mock
def test_iter_tickets_still_advances_the_offset_when_params_override_it(config):
    ...
    with EasyvistaClient(config) as client:
        seen = [t.rfc_number for t in client.iter_tickets(params={"offset": 0})]
    assert seen == ["I1", "I2", "I3"]


@respx.mock
def test_list_actions_keeps_its_rfc_filter_when_a_caller_passes_search(config):
...
@respx.mock
def test_get_api_spec_accepts_a_201_response(config):
    ...
    with EasyvistaClient(config) as client:
        document = client.get_api_spec()
    assert document["info"]["description"] == "EV REST API - 2025.3"


@respx.mock
def test_get_api_spec_honours_a_custom_path(config):
    ...
    with EasyvistaClient(config) as client:
        client.get_api_spec(path="openapi.json")
    assert route.call_count == 1


@respx.mock
def test_list_reference_table_lets_a_403_propagate(config):
    ...
    with EasyvistaClient(config) as client:
        with pytest.raises(EasyvistaAuthError):
            client.list_reference_table("catalog-requests")


@respx.mock
def test_list_reference_table_returns_a_search_result_with_counts(config):
    ...
    with EasyvistaClient(config) as client:
        page = client.list_reference_table("urgency")
...
@respx.mock
def test_discover_status_populates_the_guid_from_a_ticket_sample(config):
    ...
    with EasyvistaClient(config) as client:
        found = client.discover("STATUS", sample_size=5)
...
@respx.mock
def test_discover_a_routeless_name_never_calls_a_reference_route(config):
    ...
    with EasyvistaClient(config) as client:
        found = client.discover("IMPACT", sample_size=10)
...
@respx.mock
def test_discover_strategy_reference_lets_a_403_raise(config):
    respx.get(f"{ROOT}/status").mock(return_value=httpx.Response(403))
    with EasyvistaClient(config) as client:
        with pytest.raises(EasyvistaAuthError):
            client.discover("STATUS", strategy="reference")


@respx.mock
def test_discover_auto_falls_back_to_the_sample_on_a_denied_route(config):
    respx.get(f"{ROOT}/status").mock(return_value=httpx.Response(403))
    respx.get(f"{ROOT}/requests").mock(
        return_value=httpx.Response(
            200,
...
def test_discover_strategy_reference_refuses_a_routeless_name(config):
    ...
    with EasyvistaClient(config) as client:
        with pytest.raises(ValueError, match="no reference route"):
            client.discover("IMPACT", strategy="reference")


def test_discover_refuses_an_unknown_strategy(config):
    with EasyvistaClient(config) as client:
        with pytest.raises(ValueError, match="strategy="):
            client.discover("STATUS", strategy="guess")


@respx.mock
def test_describe_instance_names_every_gap_and_still_returns_a_profile(config):
...
@respx.mock
def test_describe_instance_records_a_truncated_table(config):
    ...
    with EasyvistaClient(config) as client:
        profile = client.describe_instance(names=["STATUS"], sample_size=1)
    assert profile.unavailable["STATUS"].startswith("truncated")
    assert profile.references["STATUS"]  # and the rows still came back


@respx.mock
def test_describe_instance_returns_a_profile_when_everything_is_denied(config):
