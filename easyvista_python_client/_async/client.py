...
class AsyncEasyvistaClient:
    ...
    # --- instance discovery --------------------------------------------------
    async def get_api_spec(self, *, path: str = SWAGGER_PATH) -> dict[str, Any]:
    ...
    async def list_reference_table(
        self,
        path: str,
        *,
        search: str | None = None,
        ...
        for rfc in rfcs:
            records.extend(
                [
                    ...
                    async for a in self.iter_actions(
    ...
    async def discover(
        ...
        **``STATUS`` also gets its GUID, and only from a sample.** A
        ...
        if strategy != "sample" and source.reference_path is not None:
            try:
                page = await self.list_reference_table(
        ...
        if needs_sample or wants_guid:
            records = await self._sample_records(
                source,
    ...
    async def describe_instance(
        ...
        **It samples once, not once per name.** Every name that needs sampling is
        ...
        if include_spec:
            try:
                document = await self.get_api_spec()
